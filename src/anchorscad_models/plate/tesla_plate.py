...
@ad.shape
@ad.datatree
class TeslaLogo(ad.CompositeShape):
    """
    A shape that imports an SVG path and extrudes it.
    """
    
    error_handler: ad.SVGErrorHandler = ad.dtfield(default_factory=lambda: ad.DefaultSVGErrorHandler())
    path_id: str = ad.dtfield(default='tesla', doc='The ID of the path to import')
    target_bounds: tuple[float, float] = ad.dtfield(default=(30, 30), doc='The target bounds of the shape')
    svg_input: str = ad.dtfield(TESLA_SVG, doc='The SVG input to import')
    svg_path_node: ad.Node[ad.Path] = ad.ShapeNode(ad.svg_to_path)
    path: ad.Path = ad.dtfield(self_default=lambda s: s.svg_path_node())
    h: float = ad.dtfield(1, doc='Height of the shape')
    extrude_node: ad.Node[ad.LinearExtrude] = ad.ShapeNode(ad.LinearExtrude)
    
    EXAMPLE_SHAPE_ARGS = ad.args()
    EXAMPLE_ANCHORS = (
        ad.surface_args(('tesla', 'centre_rhs'), 0),
    )
    
    def build(self) -> ad.Maker:
        shape = self.extrude_node()
        maker = shape.solid('extrusion').at(('tesla', 'centre_rhs'), 0)
        return maker
    
    def size(self) -> tuple[float, float, float]:
        extents = np.array(self.path.extents())
...
@ad.shape
@ad.datatree
class TeslaPlate(ad.CompositeShape):
    """
    A shape that imports a Tesla logo and extrudes it.
    """
    logo_node: ad.Node[TeslaLogo] = ad.ShapeNode(TeslaLogo, prefix='logo_')
    logo: TeslaLogo = ad.dtfield(self_default=lambda s: s.logo_node())
    logo_size: tuple[float, float, float] = ad.dtfield(
        self_default=lambda s: s.logo.size(), doc='The size of the logo')
    h: float = ad.dtfield(1.5, doc='Height of the shape')
    bsb_bevel_radius: float = ad.dtfield(3, doc='The bevel radius of the box side bevels')
    bsb_size: tuple[float, float, float] = ad.dtfield(
        self_default=lambda s: (
            s.logo_size[0] + s.bsb_bevel_radius, 
            s.logo_size[1] + s.bsb_bevel_radius, 
            s.h), doc='The size of the plate')
    box_side_bevels_node: ad.Node[BoxSideBevels] = ad.ShapeNode(BoxSideBevels, prefix='bsb_')
    
    def build(self) -> ad.Maker:
        bsb = self.box_side_bevels_node()
        maker = bsb.solid('plate').material(ad.Material("plate")).colour("black").at('face_centre', 'top')
        logo_maker = self.logo.solid('logo').material(ad.Material("logo")).colour("red").at()
        maker.add_at(logo_maker, 'face_centre', 'top', post=ad.ROTX_90)
        return maker
